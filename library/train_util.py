...
def get_optimizer(args, params_to_optimize):
    ...
    if args.use_8bit_adam:
        assert not args.use_lion_optimizer, "both option use_8bit_adam and use_lion_optimizer are specified"
    ...
    if args.fused_backward_pass:
        ...
        assert args.gradient_accumulation_steps == 1, "fused_backward_pass does not work with gradient_accumulation_steps > 1"
    ...
    if not params_to_optimize:
        raise ValueError("optimizer got an empty parameter list. This may be due to incorrect parameter grouping in train_network.py.")
    if isinstance(params_to_optimize, list) and all(isinstance(g, dict) for g in params_to_optimize):
        params_to_optimize = [g for g in params_to_optimize if g.get('params')]
        if not params_to_optimize:
            raise ValueError("optimizer got an empty parameter list after filtering empty groups in train_network.py.")
    ...
    if optimizer_type == "lion":
        try:
            ...
            raise ImportError("lion-pytorch is not installed. Please install it with 'pip install lion-pytorch'")
        logger.info(f"use Lion optimizer | {optimizer_kwargs}")
        optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)
        ...
        bnb_optimizer_map = {
            "adamw8bit": bnb.optim.AdamW8bit, "lion8bit": getattr(bnb.optim, 'Lion8bit', None),
            "pagedadamw8bit": getattr(bnb.optim, 'PagedAdamW8bit', None), "pagedlion8bit": getattr(bnb.optim, 'PagedLion8bit', None),
            "sgdnesterov8bit": bnb.optim.SGD8bit
        }
        optimizer_class = bnb_optimizer_map.get(optimizer_type)
        
        if optimizer_class:
            logger.info(f"use {optimizer_class.__name__} optimizer | {optimizer_kwargs}")
            if optimizer_type == "sgdnesterov8bit":
                if "momentum" not in optimizer_kwargs: optimizer_kwargs["momentum"] = 0.9
                optimizer_kwargs["nesterov"] = True
            optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)
        else:
            raise ValueError(f"Unknown or unsupported 8bit optimizer type: {optimizer_type}. Check your bitsandbytes version.")

    elif optimizer_type in ["pagedadamw", "pagedadamw32bit"]:
        try:
            import bitsandbytes as bnb
            optimizer_class = getattr(bnb.optim, 'PagedAdamW32bit' if '32bit' in optimizer_type else 'PagedAdamW', None)
        except (ImportError, AttributeError):
            raise ImportError("bitsandbytes 0.39.0 or later is required for Paged optimizers.")
        if optimizer_class is None:
            raise ValueError(f"Unsupported Paged optimizer type: {optimizer_type}. Check your bitsandbytes version.")
        logger.info(f"use {optimizer_class.__name__} optimizer | {optimizer_kwargs}")
        optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)
        ...
        optimizer = optimizer_class(params_to_optimize, lr=lr, nesterov=True, **optimizer_kwargs)

    elif optimizer_type.startswith("dadapt") or optimizer_type == "prodigy":
        if isinstance(params_to_optimize, list) and all(isinstance(p, dict) for p in params_to_optimize):
             if len(params_to_optimize) > 1 and not optimizer_kwargs.get('split_groups', False):
                 logger.warning(f"D-Adaptation/Prodigy with multiple groups may not work as intended without 'split_groups=True'.")
        ...
        if optimizer_type.startswith("dadapt"):
            try:
                ...
                raise ImportError("dadaptation is not installed. Please install it with 'pip install dadaptation'")
            
            optimizer_map = {
                "dadaptation": experimental.DAdaptAdamPreprint, "dadaptadampreprint": experimental.DAdaptAdamPreprint,
                "dadaptadagrad": dadaptation.DAdaptAdaGrad, "dadaptadam": dadaptation.DAdaptAdam,
                "dadaptadan": dadaptation.DAdaptAdan, "dadaptadanip": experimental.DAdaptAdanIP,
                "dadaptlion": dadaptation.DAdaptLion, "dadaptsgd": dadaptation.DAdaptSGD
            }
            optimizer_class = optimizer_map.get(optimizer_type)
            if optimizer_class is None: raise ValueError(f"Unknown DAdaptation optimizer type: {optimizer_type}")
            logger.info(f"use {optimizer_class.__name__} optimizer | {optimizer_kwargs}")
            optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)
        else: # Prodigy
            try:
                ...
                raise ImportError("prodigyopt is not installed. Please install it with 'pip install prodigyopt'")
            logger.info(f"use Prodigy optimizer | {optimizer_kwargs}")
            optimizer_class = prodigyopt.Prodigy
            optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)

    elif optimizer_type == "adafactor":
        if "relative_step" not in optimizer_kwargs: optimizer_kwargs["relative_step"] = True
        ...
        if optimizer_kwargs["relative_step"]:
            if lr > 0.0: logger.warning("learning rate is used as initial_lr")
            if isinstance(params_to_optimize, list) and all(isinstance(g, dict) for g in params_to_optimize):
                if any(g.pop("lr", None) is not None for g in params_to_optimize):
                    logger.warning("unet_lr and text_encoder_lr are ignored")
                    args.unet_lr = None; args.text_encoder_lr = None
            if args.lr_scheduler != "adafactor": logger.info("use adafactor_scheduler")
    ...
    # ▼▼▼【ここが修正箇所】▼▼▼
    elif "schedulefree" in optimizer_type or "hyperfusion" in optimizer_type or "emonavi" in optimizer_type:
        optimizer_map = {}
        # 標準のschedulefreeライブラリ
        try: 
            import schedulefree as sf
            schedulefree_map = {
                "adamwschedulefree": sf.AdamWScheduleFree,
                "sgdschedulefree": sf.SGDScheduleFree,
            }
            optimizer_map.update(schedulefree_map)
        except ImportError: pass
        
        # カスタムオプティマイザ
        try:
            from library.hyper_fusion_optimizer import HyperFusionOptimizerScheduleFree
            from library.hyper_fusion_optimizerdev import devHyperFusionOptimizerScheduleFree
            from library.emonavi import EmoNavi
            import prodigyplus as pp
            custom_map = {
                "hyperfusionschedulefree": HyperFusionOptimizerScheduleFree,
                "devhyperfusionschedulefree": devHyperFusionOptimizerScheduleFree,
                "emonavi": EmoNavi,
                "prodigyplus.prodigyplusschedulefree": pp.ProdigyPlusScheduleFree,
            }
            optimizer_map.update(custom_map)
        except ImportError as e:
            logger.warning(f"Could not import custom optimizers: {e}")
            pass

        optimizer_class = optimizer_map.get(optimizer_type)
        if optimizer_class is None:
            raise ValueError(f"Unknown or not found optimizer type for schedule-free family: {optimizer_type}")
    ...
    if optimizer is None:
        optimizer_type_str = args.optimizer_type
        logger.info(f"use {optimizer_type_str} | {optimizer_kwargs}")
        if "." not in optimizer_type_str:
            optimizer_module = torch.optim
        else:
            values = optimizer_type_str.split(".")
            optimizer_module = importlib.import_module(".".join(values[:-1]))
            optimizer_type_str = values[-1]
        optimizer_class = getattr(optimizer_module, optimizer_type_str)
        optimizer = optimizer_class(params_to_optimize, lr=lr, **optimizer_kwargs)

    optimizer_name = optimizer_class.__module__ + "." + optimizer_class.__name__
    optimizer_args_str = ",".join([f"{k}={v}" for k, v in optimizer_kwargs.items()])
