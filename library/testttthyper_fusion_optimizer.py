...
# =============================================================================
# Part 1: Main Optimizer Class (with Prodigy D-Adaptation & Factored Moments)
# =============================================================================

class HyperFusionOptimizerScheduleFree(torch.optim.Optimizer):
    r"""
    The ultimate experimental optimizer, fusing Schedule-Free, RAdam, Cautious,
    Prodigy (D-Adaptation) for automatic learning rate scaling, and optional
    ...
    def __init__(self,
                 params,
                 lr: float = 1.0, # Default to 1.0, as Prodigy will handle the scaling.
                 betas: tuple[float, float] = (0.9, 0.99), # ProdigyPlusScheduleFree uses 0.99 for beta2
                 eps: float = 1e-8,
                 weight_decay: float = 0,
                 # --- Schedule-Free Options (warmup_steps is now less critical if use_radam_rectify=True) ---
                 warmup_steps: int = 0,
                 r: float = 0.0,
                 ...
                 use_decoupled_wd: bool = True, # Decoupled is generally better
                 # --- Prodigy D-Adaptation (New Options) ---
                 use_prodigy: bool = False,
                 d0: float = 1e-6,
                 d_coef: float = 1.0,
                 beta3: Optional[float] = None,
                 growth_rate: float = float('inf'),
                 # --- Factored Moments (Memory-saving, New Option) ---
                 factored: bool = False,
                 # --- ADOPT option ---
                 use_adopt_denominator: bool = False,
                 # --- DeMo, FALCON, GEACS, Cautious ... (Existing Options) ---
        ...
        defaults = dict(lr=lr,
                        ...
                        train_mode=True, # Start in train mode
                        weight_sum=0.0,
                        lr_max=-1.0,
        ...
        if use_prodigy and use_radam_rectify:
            print("Warning: Using Prodigy (use_prodigy=True) with RAdam rectification (use_radam_rectify=True) can "
                  "significantly slow down the adaptation of 'd'. Consider disabling RAdam if 'd' fails to grow.")
        if factored and use_adabelief:
            print("Warning: Using factored second moments (factored=True) with AdaBelief is experimental and may not be stable.")
        ...
        if use_demostyle_update or use_falcon:
            if isinstance(compression_chunk, tuple): processed_chunk = compression_chunk[0]
            else: processed_chunk = compression_chunk
    ...
    def _update_d(self, group, d_numerator_new, d_denom):
        """Helper function to update Prodigy's d."""
        d_coef = group['d_coef']
    ...
    def _init_state(self, p, group, state):
        """A robust state initialization helper."""
        if 'z' not in state:
        ...
        if group['use_prodigy']:
            if 'p0' not in state:
                state['p0'] = p.data.flatten().detach().clone()
            if 's' not in state:
                state['s'] = torch.zeros_like(p.data.flatten()).detach()
        
        # Other state initializations can be added here
        if group['cautious'] and group['cautious_hybrid'] and 'exp_avg_res' not in state:
            state['exp_avg_res'] = torch.zeros_like(p.data)
        if group['use_demostyle_update'] and 'delta' not in state:
            state['delta'] = torch.zeros_like(p.data)
        if group['use_geacs'] and 'exp_avg_entropy' not in state:
            state['exp_avg_entropy'] = torch.tensor(0.0, device=p.device)
        ...
        for group in self.param_groups:
            ...
            if group['use_radam_rectify']:
                ...
                if N_sma_max > 0:
                    ...
                    if N_sma < 5:
                        ...
                        if r_t_den > 0: radam_rectify_term = math.sqrt(r_t_num / r_t_den)
            ...
            if is_radam_warmup:
                for p in group['params']:
                    ...
                    self._init_state(p, group, state) # Use the robust helper

                    exp_avg = state['exp_avg']
                    exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)
                    if not group['factored']:
            ...
            exp_avg_sqs, exp_avg_sq_rows, exp_avg_sq_cols = [], [], []
            d_denom_total = 0.0
            d_numerator_new_total = 0.0

            is_complex_path = (group['use_prodigy'] or group['use_demostyle_update'] or group['use_falcon'] or group['use_geacs'] or group['cautious'] or group['factored'])
            ...
            if not params_with_grad: group['k'] += 1; continue
            ...
            if not group['factored']:
                if group['use_adopt_denominator'] and k > 0:
                    ...
                    if group['use_adabelief']:
                        temp_exp_avg_sqs_list = []
                        for i in range(len(grads_original)):
                            s_t = (grads_original[i] - exp_avgs[i]).pow(2).add_(eps)
                            temp_exp_avg_sqs_list.append(exp_avg_sqs[i].clone().mul_(beta2).add_(s_t, alpha=1-beta2))
                        denom = torch._foreach_sqrt(temp_exp_avg_sqs_list)
                    else:
                        temp_exp_avg_sqs = torch._foreach_mul(exp_avg_sqs, beta2)
                        torch._foreach_addcmul_(temp_exp_avg_sqs, grads_original, grads_original, value=1-beta2)
                        denom = torch._foreach_sqrt(temp_exp_avg_sqs)
                torch._foreach_add_(denom, eps)
                grads_for_update = torch._foreach_div(grads_original, denom)
            else:
                grads_for_update = [torch.empty_like(g) for g in grads_original]

            if not group['factored'] and not group['use_decoupled_wd'] and group['weight_decay'] != 0:
                torch._foreach_add_(grads_for_update, [p.data for p in params_with_grad], alpha=effective_lr * group['weight_decay'])
            
            if is_complex_path:
                final_update_vectors = []
                for i in range(len(params_with_grad)):
                    p, grad_orig, state = params_with_grad[i], grads_original[i], self.state[p]
                    y, z = p.data, zs[i]
                    
                    if group['factored']:
                        r, c = exp_avg_sq_rows[i], exp_avg_sq_cols[i]
                        grad_sq = grad_orig.pow(2)
                        if group['use_adabelief']:
                            grad_sq = (grad_orig - exp_avgs[i]).pow(2)
                        ...
                        denom_approx = denom_rc.div_(r.mean(dim=-1, keepdim=True).unsqueeze(-1).clamp_min(eps)).sqrt_().add_(eps)
                        grad_normalized = grad_orig.div(denom_approx)
                    else:
                        grad_normalized = grads_for_update[i]

                    if group['factored'] and not group['use_decoupled_wd'] and group['weight_decay'] != 0:
                        grad_normalized.add_(y, alpha=effective_lr * group['weight_decay'])

                    base_update_vec = grad_normalized
                    # ... (FALCON, DeMo, GEACS logic移植 - 省略) ...
                    
                    if group['cautious']:
                        u = (y - z).mul(ckp1).add(base_update_vec, alpha=adaptive_y_lr)
                        if group['cautious_hybrid']:
                            exp_avg_res = state['exp_avg_res']; res = y - exp_avg_res
                            wh, wu = group['cautious_hybrid_weights']; h = res.mul(wh).add(u, alpha=wu)
                            suppression_factor = torch.sigmoid(-h * u).mul(2); y.sub_(u.mul(suppression_factor))
                            exp_avg_res.mul_(group['cautious_came_beta']).add(y, alpha=1-group['cautious_came_beta'])
                        else:
                            mask = (u * grad_orig > 0).to(grad_orig.dtype); y.sub_(u.mul(mask))
                    else:
                        y.lerp_(end=z, weight=ckp1).sub_(base_update_vec, alpha=adaptive_y_lr)
                    final_update_vectors.append(base_update_vec)
            else:
                y_list = [p.data for p in params_with_grad]; torch._foreach_mul_(y_list, 1.0 - ckp1)
                torch._foreach_add_(y_list, zs, alpha=ckp1); torch._foreach_sub_(y_list, grads_for_update, alpha=adaptive_y_lr)
            ...
            if not group['factored'] and not (group['use_adopt_denominator'] and k > 0):
                if not group['use_adabelief']:
...
class TransformDCT:
    ...
    def __init__(self, param_groups, target_chunk, norm="ortho"):
        self.target_chunk = target_chunk; self.shape_dict = dict(); self.f_dict = dict(); self.b_dict = dict()
            ...
            for p in group["params"]:
                ...
                for s in s_list:
                    ...
                    if sc not in self.f_dict:
                        I = torch.eye(sc, device=p.device, dtype=p.dtype); self.f_dict[sc] = _dct(I, norm=norm); self.b_dict[sc] = _idct(I, norm=norm)
    @torch.no_grad()
    ...
    def encode(self, x):
        if len(x.shape) > 1:
            n1 = self.shape_dict.get(x.shape[0], _get_smaller_split(x.shape[0], self.target_chunk)); n2 = self.shape_dict.get(x.shape[1], _get_smaller_split(x.shape[1], self.target_chunk))
            n1w = self.f_dict[n1].to(x.device, non_blocking=True); n2w = self.f_dict[n2].to(x.device, non_blocking=True)
            x = rearrange(x, "(y h) (x w) -> y h x w", h=n1, w=n2); x = self.einsum_2d(x, n1w, n2w)
        else:
            n1 = self.shape_dict.get(x.shape[0], _get_smaller_split(x.shape[0], self.target_chunk)); n1w = self.f_dict[n1].to(x.device, non_blocking=True)
            x = rearrange(x, "(x w) -> x w", w=n1); x = self.einsum_2d(x, n1w)
        return x
    @torch.no_grad()
    def decode(self, x):
        if len(x.shape) > 2:
            n1, n2 = x.shape[2], x.shape[3]; n1w = self.b_dict[n1].to(x.device, non_blocking=True); n2w = self.b_dict[n2].to(x.device, non_blocking=True)
            x = self.einsum_2d_t(x, n1w, n2w); x = rearrange(x, "y h x w -> (y h) (x w)")
        else:
            n1 = x.shape[1]; n1w = self.b_dict[n1].to(x.device, non_blocking=True)
            x = self.einsum_2d_t(x, n1w); x = rearrange(x, "x w -> (x w)")
        return x
...
class CompressDCT:
    ...
    def compress(self, x, topk):
        ...
        totalk = x.shape[-1]; topk_clamped = self._clamp_topk(x, topk)
        idx = torch.topk(x.abs(), k=topk_clamped, dim=-1, largest=True, sorted=False).indices; val = torch.gather(x, dim=-1, index=idx)
    ...
    def decompress(self, p, idx, val, xshape):
        ...
        else: x = torch.zeros(xshape, device=p.device, dtype=p.dtype)
        x.scatter_(dim=-1, index=idx, src=val)
        if len(xshape) > 2: x = rearrange(x, "y x (h w) -> y x h w", h=xshape[2])
        return x

def _dct_fft_impl(v): return torch.view_as_real(torch.fft.fft(v, dim=1))
def _idct_irfft_impl(V): return torch.fft.irfft(torch.view_as_complex(V), n=V.shape[1], dim=1)
def _dct(x, norm=None):
    x_shape = x.shape; N = x_shape[-1]; x = x.contiguous().view(-1, N)
    v = torch.cat([x[:, ::2], x[:, 1::2].flip([1])], dim=1); Vc = _dct_fft_impl(v)
    k = -torch.arange(N, dtype=x.dtype, device=x.device)[None, :] * math.pi / (2 * N)
    W_r, W_i = torch.cos(k), torch.sin(k); V = Vc[:, :, 0] * W_r - Vc[:, :, 1] * W_i
    if norm == "ortho": V[:, 0] /= math.sqrt(N) * 2; V[:, 1:] /= math.sqrt(N / 2) * 2
    V = 2 * V.view(*x_shape); return V
def _idct(X, norm=None):
    x_shape = X.shape; N = x_shape[-1]; X_v = X.contiguous().view(-1, x_shape[-1]) / 2
    if norm == "ortho": X_v[:, 0] *= math.sqrt(N) * 2; X_v[:, 1:] *= math.sqrt(N / 2) * 2
    k = torch.arange(x_shape[-1], dtype=X.dtype, device=X.device)[None, :] * math.pi / (2 * N)
    W_r, W_i = torch.cos(k), torch.sin(k); V_t_r = X_v; V_t_i = torch.cat([X_v[:, :1] * 0, -X_v.flip([1])[:, :-1]], dim=1)
    V_r = V_t_r * W_r - V_t_i * W_i; V_i = V_t_r * W_i + V_t_i * W_r; V = torch.cat([V_r.unsqueeze(2), V_i.unsqueeze(2)], dim=2)
    v = _idct_irfft_impl(V); x = v.new_zeros(v.shape); x[:, ::2] += v[:, : N - (N // 2)]; x[:, 1::2] += v.flip([1])[:, : N // 2]; return x.view(*x_shape)
...
def _get_divisors(n):
    if n <= 0: return [];
    if n == 1: return [1]
    prime_factors = _get_prime_divisors(n); divs = {1}
