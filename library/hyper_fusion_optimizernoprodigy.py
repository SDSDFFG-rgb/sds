...
class HyperFusionOptimizerScheduleFree(torch.optim.Optimizer):
    ...
    (This version has the ADOPT bug fixed.)
    """
    def __init__(self,
                 params,
                 lr: float = 0.0025,
        ...
        defaults = dict(lr=lr,
                        ...
                        use_decoupled_wd=use_decoupled_wd, use_adopt_denominator=use_adopt_denominator,
                        use_demostyle_update=use_demostyle_update, compression_decay=compression_decay,
                        compression_topk=compression_topk, compression_chunk=compression_chunk,
                        clipping_threshold=clipping_threshold, cautious=cautious,
                        cautious_hybrid=cautious_hybrid, cautious_came_beta=cautious_came_beta,
                        cautious_hybrid_weights=cautious_hybrid_weights, use_geacs=use_geacs,
                        geacs_beta=geacs_beta, geacs_temperature=geacs_temperature,
                        geacs_sensitivity=geacs_sensitivity, use_falcon=use_falcon,
                        falcon_freq_bands=falcon_freq_bands, falcon_scales=falcon_scales,
        ...
        if use_demostyle_update: self.compress = CompressDCT()
        ...
        for group in self.param_groups:
            if group['train_mode']:
                for p in group['params']:
                    state = self.state[p]
                    if 'z' in state: p.data.lerp_(end=state['z'], weight=1 - 1 / group['betas'][0])
    ...
    def step(self, closure: Callable | None = None):
        ...
        for group in self.param_groups:
            # (準備フェーズ、RAdam Warmup、共通係数計算は変更なし)
            k, beta1, beta2, eps = group['k'], group['betas'][0], group['betas'][1], group['eps']
            is_radam_warmup = False; radam_rectify_term = 1.0
            if group['use_radam_rectify']:
                beta2_t = beta2 ** (k + 1); N_sma_max = 2 / (1 - beta2) - 1
                if N_sma_max > 0:
                    N_sma = N_sma_max - 2 * (k + 1) * beta2_t / (1 - beta2_t) if (1 - beta2_t) > 0 else N_sma_max
                    if N_sma < 5: is_radam_warmup = True
                    else:
                        r_t_num = (N_sma - 4) * (N_sma - 2) * N_sma_max; r_t_den = (N_sma_max - 4) * (N_sma_max - 2) * N_sma
                        if r_t_den > 0: radam_rectify_term = math.sqrt(r_t_num / r_t_den)
            if is_radam_warmup:
                for p in group['params']:
                    if p.grad is None: continue
                    state = self.state[p]
                    if 'exp_avg' not in state: state['z'] = torch.clone(p.data); state['exp_avg'] = torch.zeros_like(p.data); state['exp_avg_sq'] = torch.zeros_like(p.data)
                    grad, exp_avg, exp_avg_sq = p.grad.data, state['exp_avg'], state['exp_avg_sq']
                    exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)
                    if group['use_adabelief']: s_t = (grad - exp_avg).pow(2).add_(eps); exp_avg_sq.mul_(beta2).add_(s_t, alpha=1 - beta2)
                    else: exp_avg_sq.mul_(beta2).addcmul_(grad, grad, value=1-beta2)
                group['k'] += 1; continue
            
            if k < group['warmup_steps']: sched = (k + 1) / group['warmup_steps']
            else: sched = 1.0
            bias_correction2 = 1 - beta2 ** (k + 1); effective_lr = group['lr'] * sched * math.sqrt(bias_correction2)
            if group['use_radam_rectify']: effective_lr *= radam_rectify_term
            lr_max = group['lr_max'] = max(effective_lr, group['lr_max']); weight = ((k + 1) ** group['r']) * (lr_max ** group['weight_lr_power'])
            weight_sum = group['weight_sum'] = group['weight_sum'] + weight; ckp1 = weight / weight_sum if weight_sum > 0 else 0
            adaptive_y_lr = effective_lr * (1 - beta1 * (1 - ckp1))

            # (データ収集フェーズは変更なし)
            params_with_grad, grads_original, exp_avgs, exp_avg_sqs, zs = [], [], [], [], []
            is_complex_option_enabled = (group['use_demostyle_update'] or group['use_falcon'] or group['use_geacs'] or group['cautious'])
            for p in group['params']:
                if p.grad is None: continue
                params_with_grad.append(p); grads_original.append(p.grad.data)
                state = self.state[p]
                if 'z' not in state:
                    state['z'] = torch.clone(p.data); state['exp_avg'] = torch.zeros_like(p.data); state['exp_avg_sq'] = torch.zeros_like(p.data)
                ...
                exp_avgs.append(state['exp_avg']); exp_avg_sqs.append(state['exp_avg_sq']); zs.append(state['z'])
            ...
            # --- ★★★ ADOPTバグ修正 箇所1 ★★★ ---
            # ここでは分母を計算するだけ。exp_avg_sqsのリスト自体は変更しない
            if group['use_adopt_denominator'] and k > 0:
                # ADOPTが有効なら、古いexp_avg_sqsをそのまま使う
                denom = torch._foreach_sqrt(exp_avg_sqs)
                torch._foreach_add_(denom, eps)
            else:
                # ADOPTが無効なら、現在の勾配で更新した値で分母を計算
                if group['use_adabelief']:
                    temp_denom_list = []
                    for i in range(len(grads_original)):
                        s_t = (grads_original[i] - exp_avgs[i]).pow(2).add_(eps)
                        temp_exp_avg_sq = exp_avg_sqs[i].clone().mul_(beta2).add_(s_t, alpha=1-beta2)
                        temp_denom_list.append(temp_exp_avg_sq.sqrt_().add_(eps))
                    denom = temp_denom_list
                else:
                    # clone()の代わりに一時変数temp_exp_avg_sqsを使用し、元のexp_avg_sqsを変更しない
            ...
            if is_complex_option_enabled:
                ...
                for i in range(len(params_with_grad)):
                    ...
                    if group['use_demostyle_update']:
                        delta = state['delta']; encoded_g = self.transform.encode(grad_normalized); idx, val, xshape, totalk = self.compress.compress(encoded_g, group['compression_topk'])
                        compressed_g = self.compress.decompress(grad_normalized, idx, val, xshape); immediate_update = self.transform.decode(compressed_g)
                        delta.mul_(group['compression_decay']).add_(immediate_update, alpha=1-group['compression_decay']); torch.clamp(delta, -group['clipping_threshold'], group['clipping_threshold'], out=delta)
                        base_update_vec = delta
                    elif apply_falcon_to_this_param:
                        encoded_g = self.transform.encode(grad_normalized); low_band, high_band = group['falcon_freq_bands']; low_scale, mid_scale, high_scale = group['falcon_scales']
                        mask_low = torch.ones_like(encoded_g); mask_mid = torch.ones_like(encoded_g); mask_low[..., low_band:] = 0
                        mask_mid[..., :low_band] = 0; mask_mid[..., high_band:] = 0; mask_high = 1 - (mask_low + mask_mid)
                        base_update_vec = self.transform.decode(encoded_g * (mask_low * low_scale + mask_mid * mid_scale + mask_high * high_scale))
                    if group['use_geacs']:
                        exp_avg_entropy = state['exp_avg_entropy']; entropy = torch.sum(-torch.log(denom[i]) / math.log(denom[i].numel())).item()
                        exp_avg_entropy.mul_(group['geacs_beta']).add_(entropy, alpha=1-group['geacs_beta']); confidence = torch.sigmoid((exp_avg_entropy - group['geacs_sensitivity']) / group['geacs_temperature'])
                    ...
                    if group['cautious']:
                        ...
                        if group['cautious_hybrid']:
                            ...
                            y.sub_(u.mul(suppression_factor)); exp_avg_res.mul_(group['cautious_came_beta']).add(y, alpha=1-group['cautious_came_beta'])
                        else: mask = (u * grad_orig > 0).to(grad_orig.dtype); y.sub_(u.mul(mask))
                    else: y.lerp_(end=z, weight=ckp1).sub_(base_update_vec, alpha=adaptive_y_lr)
            ...
            # --- ★★★ ADOPTバグ修正 箇所2 ★★★ ---
            # このブロックで、*次の*ステップのためにexp_avg_sqsを必ず更新する
            if group['use_adabelief']:
                # AdaBeliefはforループで処理する必要がある
                for i in range(len(grads_original)):
                    s_t = (grads_original[i] - exp_avgs[i]).pow(2).add_(eps)
...
# (Helper Classes and Functionsは変更なし)
# ... (TransformDCT, CompressDCT, _dct, _idct, etc.)
# ... (以下、元のコードのまま)
# ...
class TransformDCT:
    @torch.no_grad()
